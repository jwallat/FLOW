...
public class SceneGestures {
	...
	private EventHandler<ScrollEvent> onScrollEventHandler = new EventHandler<ScrollEvent>() {
		...
		public void handle(ScrollEvent event) {
			...
			double scale = pane.getScale(); // currently we only use Y, same
											// value is used for X
			...
			double dx = (event.getSceneX()
					- (pane.getBoundsInParent().getWidth() / 2 + pane.getBoundsInParent().getMinX()));
			double dy = (event.getSceneY()
					- (pane.getBoundsInParent().getHeight() / 2 + pane.getBoundsInParent().getMinY()));
