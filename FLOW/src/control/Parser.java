...
/**
 * XML-Parser für die XML-Dateien aus dem Tool. Es wird die SAX Library zum
 * Parsen der XML verwendet.
 * 
 * @author Jonas Wallat
 *
 */
public class Parser extends DefaultHandler {
	...
	/**
	 * Funktion die das Parsen der Datei beginnt. Wenn showLog = true wird auch der
	 * ErrorLog nach dem Parsen ausgegeben.
	 * 
	 */
	public void parse() {
		try {
	...
	/**
	 * Funktion des SAX-Parsers. Geht die einzelnen Elemente durch und erzeugt
	 * entweder neue Member oder Edges.
	 * 
	 */
	...
	/**
	 * Erzeugt aus den uebergebenen Informationen eine neue Person/Document/Activity
	 * und fuegt sie dem Network hinzu, falls die Informationen vollstaendig sind.
	 * Ist das nicht der Fall wird dies in den ErrorLog geschrieben.
	 * 
	 * @param uri
	 *            Die namespace uri
	 * @param localName
	 *            local name (ohne praefix)
	 * @param qName
	 *            qualified name (mit praefix)
	 * @param atts
				...
				if (atts.getValue("x") != "") {
					if (atts.getValue("y") != "") {
						if (atts.getValue("type").equals("person")) {
							newElement = new Person(atts.getValue("name"), "person",
									Integer.parseInt(atts.getValue("id")), Integer.parseInt(atts.getValue("x")),
									Integer.parseInt(atts.getValue("y")));
						} else if (atts.getValue("type").equals("document")) {
							newElement = new Document(atts.getValue("name"), "document",
									Integer.parseInt(atts.getValue("id")), Integer.parseInt(atts.getValue("x")),
									Integer.parseInt(atts.getValue("y")));
						} else if (atts.getValue("type").equals("activity")) {
							newElement = new Activity(atts.getValue("name"), "acitivity",
									Integer.parseInt(atts.getValue("id")), Integer.parseInt(atts.getValue("x")),
									Integer.parseInt(atts.getValue("y")));
						} else {
	 ...
	 * Network hinzu, falls die Informationen vollstaendig sind. Ist das nicht der
	 * Fall wird dies in den ErrorLog geschrieben.
	 * 
	 * @param uri
	 *            Die namespace uri
	 * @param localName
